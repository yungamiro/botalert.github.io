...
export class DatabaseService {
  ...
  async updateMonitoringTarget(id: string, updates: Partial<MonitoringTarget>): Promise<void> {
    ...
    await run(`UPDATE monitoring_targets SET ${fields.join(', ')} WHERE id = ?`, values);
  }

  async deleteMonitoringTarget(id: string): Promise<void> {
    const run = promisify(this.db.run.bind(this.db));
    await run('DELETE FROM monitoring_targets WHERE id = ?', [id]);
    await run('DELETE FROM alerts WHERE target_id = ?', [id]);
  }
